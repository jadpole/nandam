...
def test_union_mut_subclasses_returns_all_leaf_classes() -> None:
    subclasses = AnimalMut._subclasses()
...
def test_union_mut_subclasses_excludes_base_class() -> None:
    subclasses = AnimalMut._subclasses()
    assert AnimalMut not in subclasses.values()
...
def test_union_mut_find_subclass_returns_correct_class() -> None:
    assert AnimalMut._find_subclass("dog") == Dog
    assert AnimalMut._find_subclass("cat") == Cat
    assert AnimalMut._find_subclass("bird") == Bird


def test_union_mut_find_subclass_returns_none_for_unknown() -> None:
    assert AnimalMut._find_subclass("fish") is None
    assert AnimalMut._find_subclass("") is None
...
def test_union_mut_from_dict_raises_for_unknown_kind() -> None:
    data = {"kind": "fish", "name": "Nemo"}
    with pytest.raises(ValueError, match="unknown"):
        AnimalMut._from_dict(data)


def test_union_mut_from_dict_raises_for_missing_kind() -> None:
    data = {"name": "Unknown"}
    with pytest.raises(ValueError, match="unknown"):
        AnimalMut._from_dict(data)
...
def test_model_validate_dict_dispatches_correctly() -> None:
    ...
    dog = AnimalMut.model_validate(dog_data)
...
def test_model_validate_json_dispatches_correctly() -> None:
    ...
    cat = AnimalMut.model_validate_json(json_str)
...
def test_serialization_roundtrip_dog() -> None:
    ...
    deserialized = AnimalMut.model_validate(serialized)
...
def test_json_roundtrip() -> None:
    ...
    deserialized = AnimalMut.model_validate_json(json_str)
...
def test_union_mut_discriminated_returns_annotated_type() -> None:
    discriminated = AnimalMut.discriminated_union()
