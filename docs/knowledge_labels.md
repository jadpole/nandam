...
- Property names are derived from observation URIs using `LabelName.try_normalize()`
...
- Previously generated labels are passed as `cached` to `generate_standard_labels()`
- The system checks `(field_name, observation_uri)` tuples against the cache
- Cached labels are **not** re-generated, even if other labels are requested
...
Labels are configured via `labels.yml` or fall back to built-in defaults.
...
|---------------|-----------------------------|------------------------------------------|
| `name`        | `LabelName`                 | Unique identifier for the field          |
...
When no `labels.yml` is present, two labels are generated by default:
